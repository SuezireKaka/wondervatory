...
public class ToolService {
	 ...
	 * listAllFromSeries(String seriesId, int page) { PagingDTO paging = new
	 * PagingDTO(page);
	 * 
	...
	public DreamPair<List<ToolVO>, PagingDTO> listAllNextTools(String seriesId, String idPath, int page) {
	...
	/*
	 * public List<CustomPropertyVO> listPropertiesOf(String objectId) { return
	 * toolMapper.listPropertiesOf(objectId); } --deprecated : 프론트앤드 테스트 종료
	 */

	public ToolVO getToolById(AccountVO askAccount, String toolId) {
		...
		resRelationList.stream().forEach(relation -> {
			relation.getCustomPropertiesList().addAll(customObjectMapper.listPropertiesOf(relation.getId()));
		});
	...
	public ToolVO saveToolDetails(AccountVO writer, ToolVO toolData) {
		...
		List<CustomEntityVO> newEntityList = entityList.stream().filter(entity -> entity.getId().startsWith("----"))
				.collect(Collectors.toList());
		List<CustomEntityVO> editedEntityList = entityList.stream().filter(entity -> entity.isEdited())
				.collect(Collectors.toList());
		...
		List<CustomRelationVO> newRelationList = relationList.stream()
				.filter(relation -> relation.getId().startsWith("----")).collect(Collectors.toList());
		List<CustomRelationVO> editedRelationList = relationList.stream().filter(relation -> relation.isEdited())
				.collect(Collectors.toList());
		...
		List<CustomObjectVO> deleteList = oldObjectList.stream().filter(obj -> !bringIdList.contains(obj.getId()))
				.collect(Collectors.toList());
	...
	private void insertNewObjects(String toolId, List<CustomEntityVO> newEntityList,
		...
		if (newEntityList.size() > 0) {
			customObjectMapper.insertObjectsToSync(toolId, "Entity",
					newEntityList.stream().map(CustomObjectVO.class::cast).collect(Collectors.toList()));
		}

		if (newRelationList.size() > 0) {
			customObjectMapper.insertObjectsToSync(toolId, "Relation",
					newRelationList.stream().map(CustomObjectVO.class::cast).collect(Collectors.toList()));
		}
	}

	private String[] grantNewIds(List<CustomEntityVO> newEntityList, List<CustomRelationVO> newRelationList) {
		...
		// 그 개수만큼 아이디를 뽑아온다
		String[] newIdArray = customObjectMapper.getNextMultiIdConcat("s_object", summonCount).split(", ");
		...
		for (int n = 0; n < summonCount; n++) {
			...
			if (n >= newEntityCount) {
				...
				newRelationList.get(n - newEntityCount).setId(grantId);
			}
			// 아니면 엔티티로 간주하고 처리
			else {
				// n번째 녀석에 해당되는 object의 id를 바꾸고
				newEntityList.get(n).setId(grantId);
			}
			// relationList 각각에 대해
