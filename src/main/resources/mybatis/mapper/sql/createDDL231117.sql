...
--이부분새로만듬--
create table t_account(--이부분새로만듬--
	id			char(4) primary key,--이부분새로만듬--
	login_id	varchar(255),--이부분새로만듬--
	pass_word	varchar(255),--이부분새로만듬--
	owner_id	char(4),--이부분새로만듬--
	response_id	char(4),--이부분새로만듬--
	nick		varchar(255),--이부분새로만듬--
	intro		varchar(255),--이부분새로만듬--
	alive		TINYINT(4) DEFAULT '1',--이부분새로만듬--
	reg_dt		timestamp default current_timestamp(),--이부분새로만듬--
	upt_dt		timestamp default current_timestamp() on update current_timestamp()--이부분새로만듬--
);--이부분새로만듬--
